...
const LAUNCHES = [
  {
    id: '1',
    name: 'Gladly AI Launch',
    tier: 'Large / XL',
    tierColor: 'bg-purple-100 text-purple-700',
    tracks: [
      { label: 'T1 Pipeline Driver', color: 'bg-blue-100 text-blue-700' },
      { label: 'T5 Retire + Replace', color: 'bg-orange-100 text-orange-700' },
    ],
    status: 'In Progress',
    statusColor: 'bg-green-100 text-green-700',
    description: 'Sidekick → Gladly AI rename. Revenue-driving launch requiring new pitch motion + change management.',
    notionBriefUrl: '#',
    planningDocUrl: '#',
    calendarCount: 3,
    targetDate: 'Apr 30',
    totalTasks: 21,
    completedTasks: 11,
    eduTotal: 11, eduDone: 6,
    enbTotal: 10, enbDone: 5,
  },
  {
    id: '2',
    name: 'Team Assist',
    ...
    tracks: [
      { label: 'T3 Unlearn / Relearn', color: 'bg-amber-100 text-amber-700' },
    ],
    status: 'In Progress',
    statusColor: 'bg-yellow-100 text-yellow-700',
    description: 'Changes how team collaboration works. Reps need to unlearn previous workflow patterns.',
    notionBriefUrl: null,
    planningDocUrl: null,
    calendarCount: 0,
    targetDate: 'May 15',
    totalTasks: 12,
    completedTasks: 3,
    eduTotal: 7, eduDone: 2,
    enbTotal: 5, enbDone: 1,
  },
]
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="px-6 py-4 border-t border-slate-200 flex items-center justify-end gap-3">
              ...
              <button
                disabled={!formName.trim() || !formTier}
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1 min-w-0 pr-8">
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  ...
                  <span className={`inline-flex items-center text-xs font-semibold px-2 py-0.5 rounded-full ${launch.tierColor}`}>{launch.tier}</span>
                  {launch.tracks.map(t => (
                    <span key={t.label} className={`inline-flex items-center text-xs font-semibold px-2 py-0.5 rounded-full ${t.color}`}>{t.label}</span>
                  ))}
                  <span className={`inline-flex items-center text-xs font-semibold px-2 py-0.5 rounded-full ${launch.statusColor}`}>{launch.status}</span>
                </div>
                <p className="text-slate-500 text-sm mb-3">{launch.description}</p>
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  {launch.calendarCount > 0 && (
                    <span className="flex items-center gap-1 text-xs text-teal-600 font-medium">
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                      {launch.calendarCount} sessions on calendar
                    </span>
                  )}
                  <span className="text-xs text-slate-400">Target: {launch.targetDate}</span>
                </div>
              </div>
              <div className="flex-shrink-0 text-right">
                <div className="text-2xl font-bold text-slate-900">{pct(launch.completedTasks, launch.totalTasks)}%</div>
                <div className="text-xs text-slate-500 mb-2">{launch.completedTasks} / {launch.totalTasks} tasks</div>
                <div className="w-32 bg-slate-100 rounded-full h-2">
                  <div className="bg-teal-500 h-2 rounded-full" style={{ width: `${pct(launch.completedTasks, launch.totalTasks)}%` }} />
                </div>
              </div>
            </div>
            <div className="mt-4 pt-4 border-t border-slate-100 grid grid-cols-2 gap-4">
              <div>
                <div className="flex justify-between text-xs text-slate-500 mb-1.5">
                  <span>📚 Education — Emily, Eliza, Gerard</span>
                  <span>{launch.eduDone} / {launch.eduTotal}</span>
                </div>
                <div className="w-full bg-slate-100 rounded-full h-1.5">
                  <div className="bg-violet-500 h-1.5 rounded-full" style={{ width: `${pct(launch.eduDone, launch.eduTotal)}%` }} />
                </div>
              </div>
              <div>
                <div className="flex justify-between text-xs text-slate-500 mb-1.5">
                  <span>⚡ Rev Enablement — Gina, Christian</span>
                  <span>{launch.enbDone} / {launch.enbTotal}</span>
                </div>
                <div className="w-full bg-slate-100 rounded-full h-1.5">
                  <div className="bg-teal-500 h-1.5 rounded-full" style={{ width: `${pct(launch.enbDone, launch.enbTotal)}%` }} />
                </div>
              </div>
            </div>
          </Link>
